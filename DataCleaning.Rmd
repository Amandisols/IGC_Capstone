...
New column for measurements, cleaning the `Focal: Salt/Ion type (if known) OR Conductivity` column for spelling and capatalization errors.
```{r}
...
DATA$SaltUnits <- ifelse(DATA$SaltUnits %in% c("uM Ca2+", "uM", "mM"), "µM", DATA$SaltUnits)

DATA$SaltUnits <- ifelse(DATA$SaltUnits %in% c("mM/l", "mM/L"), "µM/L", DATA$SaltUnits)
...
#Convert g/L to mg/L
#Convert mS/m to us/cm
# milliSiemens per cm to uS?
#Convert ppb to ppm to g/L
...
write_csv(DATA, "data/RawSalts_Corrected.csv")
