...
IONS <- DATA %>%
  filter(Measurement %in% c("NaCl", "KCl", "Na2SO4", "MgCl2", "CaCl2", "NaHCO3", "MgSO4", "NaSO4", "CaSO4", "Ca2+","CaCO3", "Cl-", "SO4 2-", "K+", "NO3-", "Mg3+", "Bicarbonate", "K2SO4", "KHCO3", "NH4+", "Na+" ,"NH4Cl", "NaF", "(NH4)2SO4", "NaNO3", "HCO3-", "iron oxide", "NH3+")) 

unique(IONS$SaltUnits)
IONS$`Minimum concentration_ppm` <- NA
IONS$`Maximum concentration_ppm` <- NA

##### ppt (is this parts per trillion? or parts per thousand? this could change)
...
mmol <- DATA %>%
  filter(SaltUnits %in% c("mmol/L")) 




#Check for stragglers (no new data). Likely a name that needs to be fixed in 'DataCleaning' and then added above to conversions. 
